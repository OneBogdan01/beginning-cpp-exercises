...
std::size_t maxWordLength(const WordCounts& wordCounts) {
    ...
    // std::views::filter(wordCounts, [&max](const auto& kv) -> bool {
    //     const auto& [key, value] = kv;
    //     if (value >= 2 && max < key.length()) {
    //         max = key.length();
    //     }
    // });
    auto view{wordCounts | filter([](const auto wordCounts) { return wordCounts.second >= 2; }) |
              transform([](const auto wordCounts) { return wordCounts.first.length(); })};

    return std::ranges::empty(view) ? 0 : std::ranges::max(view);
}
