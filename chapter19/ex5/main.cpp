...
class DeliveryTruck {
  public:
    DeliveryTruck(const Truckload& load) : m_load{load} {};
    void deliverBox() {
        auto iterator{m_load.getIterator().getLastBox()};
        for (auto& delivery : m_deliveries) {
            delivery(*iterator);
        }
        m_load.removeBox(iterator);
    };
    void registerOnDelivered(std::function<void(Box&)> func) {
        m_deliveries.emplace_back(func);
    };

  private:
    Truckload m_load;
    std::vector<std::function<void(Box&)>> m_deliveries;
};
int main()

{
    ...
    DeliveryTruck delivery{truck2};
    size_t counts{};
    delivery.registerOnDelivered(logDelivery);
    delivery.registerOnDelivered([&counts](Box& box) {
        counts += 1;
        std::println("Total of {} delivered", counts);
    });
    delivery.deliverBox();
    delivery.deliverBox();
    delivery.deliverBox();
