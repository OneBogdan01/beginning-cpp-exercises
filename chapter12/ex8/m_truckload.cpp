...
SharedBox Truckload::Iterator::getNextBox() {
    ...
    SharedBox result{m_current->m_box}; // Extract the box to return
...
bool Truckload::removeBox(SharedBox boxToRemove) {
    ...
    while (current) {
        if (current->m_box == boxToRemove) // We found the Box!
        {
            ...
            current->m_previous = nullptr; // Disconnect the current Package from the list
