...
class LinkedList {
  public:
    ...
    void swap(LinkedList& src) noexcept {
        std::swap(m_head, src.m_head);
        std::swap(m_tail, src.m_tail);
        std::swap(m_size, src.m_size);
    }
    LinkedList(LinkedList&& src) noexcept
        : m_head(src.m_head), m_tail(src.m_tail), m_size(src.m_size) {
    ...
    void push_back(T value) {
        auto* package{new Package{std::move(value)}}; // Create a new Package
  ...
  private:
    struct Package {
        ...
        Package(T value) : m_value{std::move(value)} {}
    };
